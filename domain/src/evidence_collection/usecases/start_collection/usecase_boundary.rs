pub mod request {
    ...
    use kernel_oss::values::specification::kind::Kind;
    ...
    impl StartProcedureBuilder {
        ...
        fn validate_subject(&self) -> Result<Subject, Error> {
            let nrn = self.subject_nrn.as_ref().ok_or(
                self.custom_error("The Subject NRN is required, although it was not provided."),
            )?;
            let subject_id = self.subject_id.as_ref().ok_or(
                self.custom_error("The Subject ID is required, although it was not provided."),
            )?;
        ...
        fn validate_procedure(&self) -> Result<Procedure, Error> {
            let repo_link = self.procedure_repository.as_ref().ok_or(self.custom_error(
                "The procedure repository link is required, although it was not provided.",
            ))?;
            let directory = self.procedure_directory.as_ref().ok_or(self.custom_error(
                "The procedure directory is required, although it was not provided.",
            ))?;
        ...
        fn custom_error(&self, message: &str) -> Error {
            Error::for_user(
                ...
                format!(
                    "There is an issue with your Start Procedure request. {}",
                    message
                ),
            )
        }
