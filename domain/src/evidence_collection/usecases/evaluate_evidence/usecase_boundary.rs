...
pub mod request {
    ...
    impl<'a> EvaluateEvidenceBuilder<'a> {
        ...
        pub fn try_build(&self) -> Result<EvaluateEvidence, Error> {
            let subject =
                validate_subject(self.subject_nrn, self.subject_id).map_err(custom_error)?;
            let procedure = validate_procedure(self.procedure_repo, self.procedure_dir)
    ...
    fn validate_subject(nrn: Option<&str>, id: Option<&str>) -> Result<Subject, Error> {
        ...
        Subject::try_new(nrn, id).map_err(|e| {
            Error::for_user(
                Kind::InvalidInput,
                format!(
                    "There is an issue with the Subject data you provided. {}",
                    e.message
                ),
            )
        })
    ...
    fn validate_procedure(
        ...
        let repo_link = repo_link.ok_or_else(|| {
            Error::for_user(
                Kind::InvalidInput,
        ...
        Procedure::try_new(repo_link, directory).map_err(|e| {
            Error::for_user(
                Kind::InvalidInput,
                format!(
                    "There is an issue with the Procedure data you provided. {}",
                    e.message
                ),
            )
        })
    ...
    fn validate_metadata(metadata: &Vec<(String, String)>) -> Result<MetaData, Error> {
        ...
        for (key, value) in metadata {
            valid_metadata.add(key, value).map_err(|e| {
                Error::for_user(
                    Kind::InvalidInput,
                    format!(
                        "There is an issue with the Metadata you provided. {}",
                        e.message
                    ),
                )
            })?;
    ...
    fn custom_error(error: Error) -> Error {
        Error::for_user(
            error.kind,
            format!(
                "We are unable to create a valid Evaluate Evidence request. {}",
                error.message
            ),
        )
    }
