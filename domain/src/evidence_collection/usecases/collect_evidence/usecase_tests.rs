use crate::evidence_collection::usecases::collect_evidence::usecase::{
    collect_action_evidence, CollectEvidenceRequest,
};
...
fn collect_file_success() {
    let file_name_assertion =
        move |file_name: &str, _file_data: &Vec<u8>, _target_directory: &str| {
            copy_evidence_file_gateway_file_name_assertion(
                file_name,
                _file_data,
                _target_directory,
                "file.txt",
    ...
    let result = collect_action_evidence(
        &request,
    ...
    assert!(
        ...
        "{}",
        format!(
            "An error was not expected but one was not returned: {:?}",
            result.err()
        )
    );
}
...
fn collect_file_error_action_name_error() {
    ...
    let expected_message =
        "There is an issue with the action name 'a band action name'.".to_string();
    assert!(
        error.message.starts_with(&expected_message),
        "{}",
        format!(
            "The error message '{}' did not start with the expected message '{}'",
...
fn retrieve_directory_error(_directory_name: &str) -> Result<String, Error> {
    Err(Error::for_user(
        Kind::GatewayError,
        "Some RetrieveDirectoryPath Gateway Error".to_string(),
    ))
}
...
fn retrieve_file_data_error(_file_path: &str) -> Result<(String, Vec<u8>), Error> {
    Err(Error::for_user(
        Kind::GatewayError,
        "Some RetrieveFileData Gateway Error".to_string(),
    ))
}

/*** CopyEvidenceFile Gateway Mocks & Assertions***/

fn copy_evidence_file_gateway_success(
    _file_name: &str,
    _file_data: &Vec<u8>,
...
fn copy_evidence_file_gateway_error(
    _file_name: &str,
    _file_data: &Vec<u8>,
    _target_directory: &str,
) -> Result<String, Error> {
    Err(Error::for_user(
        Kind::GatewayError,
        "Some CopyEvidenceFile Gateway Error".to_string(),
    ))
}
fn copy_evidence_file_gateway_file_name_assertion(
    file_name: &str,
    _file_data: &Vec<u8>,
    _target_directory: &str,
    ...
    assert_eq!(
        ...
        "{}",
        format!(
            "The file name '{}' was not the expected file name of '{}'.",
