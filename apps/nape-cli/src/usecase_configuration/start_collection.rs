...
pub fn factory_std_fs_git2() -> UCStartCollectionProcedure {
    move |request: StartProcedure| -> Result<ProcedureStarted, Error> {
        ...
        let state_file = nape_cli_config_file_path().map_err(|error| {
            Error::for_system(
                Kind::GatewayError,
                format!(
                    "Failed to retrieve the local NAPE state configuration file . {}",
                    error.message
                ),
            )
        })?;

        write_to_filesystem(&app_state, serialize_to_yaml, &state_file).map_err(|error| {
            Error::for_system(
                Kind::GatewayError,
                format!(
                    "Failed to write the CLIAppState to the filesystem. {}",
                    error.message
                ),
            )
        })?;
...
fn generate_directory_list(request: &StartProcedure) -> Result<DirectoryList, Error> {
    ...
    let report_instance_home_directory =
        format!("{}/{}", encoded_directory_name, collection_directory_name);
    ...
    CLIAppState::builder()
        ...
        .map_err(|error| {
            Error::for_system(
                Kind::GatewayError,
                format!(
                    "Failed to build the CLIAppState from the Start Collection Procedure. {}",
                    error.message
                ),
            )
        })
