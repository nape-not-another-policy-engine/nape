...
impl CLIAppState {
    ...
    pub fn try_directory_list(self) -> Result<DirectoryList, Error> {
        DirectoryList::try_from_hashmap(self.directories).map_err(|error| {
            Error::for_system(
                Kind::GatewayError,
                format!(
                    "Failed to convert the CLIAppState directories to a DirectoryList. {}",
                    error.message
                ),
            )
        })
...
impl CLIAppStateBuilder {
    ...
    fn validate_metadata(&self) -> Result<HashMap<String, String>, Error> {
        Ok(self
            ...
            .data
            .iter()
            .map(|(name, desc)| (name.value.clone(), desc.value.clone()))
            .collect())
    }
    fn validate_directory_list(&self) -> Result<HashMap<String, String>, Error> {
        Ok(self
            ...
            .as_ref()
            .ok_or(
                self.custom_error("A directory List is required, although one was note provided."),
            )?
            .paths
            .iter()
            .map(|(path_name, path_value)| (path_name.clone(), path_value.clone()))
            .collect())
    }
    ...
    fn custom_error(&self, message: &str) -> Error {
        Error::for_system(
            Kind::InvalidInput,
            format!(
                "There is an issue establishing the CLI Application State. {}",
                message
            ),
        )
    }
