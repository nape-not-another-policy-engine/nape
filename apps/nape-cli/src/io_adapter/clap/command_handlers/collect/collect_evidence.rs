...
fn extract_arguments(matches: &ArgMatches) -> Result<CollectEvidenceRequest<'_>, Error> {
    let action_name = matches
        .get_one::<String>("control-activity-name")
        .ok_or(Error::for_user(
            Kind::InvalidInput,
            String::from("The control action name is required."),
        ))?;
    let file_path = matches
        .get_one::<String>("evidence-file-path")
        .ok_or(Error::for_user(
            Kind::InvalidInput,
            String::from("The evidence file path is required."),
        ))?;
