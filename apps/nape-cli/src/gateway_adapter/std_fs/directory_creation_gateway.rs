...
///
pub fn create_directories_on_filesystem(
    ...
    directory_list
        ...
        .map(|(path_name, directory_path)| {
            ...
            fs::create_dir_all(fully_qualified_directory_path).map_err(|err| {
                Error::for_system(
                    Kind::GatewayError,
            ...
            let fully_qualified_directory_path_as_str =
                match fully_qualified_directory_path.to_str() {
                    Some(path) => path,
                    None => {
                        return Err(Error::for_system(
                            ...
                            format!(
                                ...
                                fully_qualified_directory_path.display()
                            ),
                        ))
                    }
                };

            updated_directory_list = updated_directory_list
                .try_add(path_name, fully_qualified_directory_path_as_str)
                .map_err(|err| {
                    Error::for_system(
                        Kind::GatewayError,
                        format!(
                            "Failed to add path ['{}', '{}'] to directory list. '{}'",
                            path_name, fully_qualified_directory_path_as_str, err.message
                        ),
                    )
                })?;
