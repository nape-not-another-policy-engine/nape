...
fn delete_source_file_success() {
    ...
    assert!(
        source_path.exists(),
        "The files was not created, check the test setup to ensure the file-to-delete was created."
    );
...
fn delete_source_error_dir_cannot_be_accessed_not_found() {
    ...
    assert!(
        result.is_err(),
        "Expected an error but one was not provided."
    );
    ...
    assert!(result_error.message.starts_with(
        "The path 'delete_source_error_dir_cannot_be_accessed_not_found' cannot be accessed. "
    ));
}
#[test]
fn delete_source_error_file_cannot_be_accessed_not_found() {
    ...
    assert_eq!(result_error.audience, Audience::System);
    assert!(result_error.message.starts_with(
        "The path 'delete_source_error_file_cannot_be_accessed_not_found.txt' cannot be accessed. "
    ));
}
#[test]
fn delete_source_error_file_cannot_be_accessed_no_permissions() {
    ...
    let source =
        "delete_source_error_file_cannot_be_accessed_no_permissions/activity/test_file.txt";
    ...
    let parent_dir = absolute_source
        .parent()
    ...
    fs::remove_dir_all("delete_source_error_file_cannot_be_accessed_no_permissions")
...
fn delete_source_error_failed_to_delete_dir() {
    ...
    assert!(result_error
        .message
        .starts_with("Failed to delete directory 'delete_source_error_failed_to_delete_dir'. "));
...
fn delete_source_error_not_file_or_dir() {
    ...
    assert!(result_error
        .message
        .starts_with("The path '/dev/null' is neither a file nor a directory."));
}
