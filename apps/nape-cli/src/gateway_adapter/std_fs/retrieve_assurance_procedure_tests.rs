...
use test_framework_oss::{canonical_path, create_file, remove};
...
fn success() {
    ...
    let file_path_buff = create_file!(
        "retrieve_assurance_procedure_success/assurance_procedure.yaml",
        &file_content
    );
    ...
    assert_eq!(
        assurance_procedure.kind,
        specification::kind::Kind::AssuranceProcedure
    );
    assert_eq!(
        assurance_procedure.procedure.nrn.value,
        "nrn:procedure:nape/software-procedures:rust-ci/sourcecode-integration"
    );
    assert_eq!(
        assurance_procedure.procedure.short.value,
        "Rust CI Assurance Procedure"
    );
    ...
    assert_eq!(
        assurance_procedure.activities.list[0].name.value,
        "peer-review"
    );
    assert_eq!(
        assurance_procedure.activities.list[0].short.value,
        "Peer Review"
    );
    ...
    assert_eq!(
        assurance_procedure.activities.list[0].actions[0].name.value,
        "at-least-two-reviewers"
    );
    assert_eq!(
        assurance_procedure.activities.list[0].actions[0]
            .short
            .value,
        "Two (2) Reviewer Approval"
    );
    assert_eq!(
        assurance_procedure.activities.list[0].actions[0]
            .description
            .value,
        "There are at least two (2) peer reviews who approved the merge request."
    );
    assert_eq!(
        assurance_procedure.activities.list[0].actions[0]
            .test
            .as_str(),
        "./activity/peer_review/at_least_two_reviewers.py"
    );
    assert_eq!(
        assurance_procedure.activities.list[0].actions[0]
            .evidence
    ...
    assert_eq!(
        assurance_procedure.activities.list[0].actions[1].name.value,
        "requester-not-a-reviewer"
    );
    assert_eq!(
        assurance_procedure.activities.list[0].actions[1]
            .short
            .value,
    ...
    assert_eq!(
        assurance_procedure.activities.list[0].actions[1]
            .test
            .as_str(),
        "./activity/peer_review/requester_not_a_reviewer.py"
    );
    assert_eq!(
        assurance_procedure.activities.list[0].actions[1]
            .evidence
