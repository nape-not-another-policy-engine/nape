...
///
pub fn retrieve_procedure_from_git(
...
fn get_head_from_repo<'a>(
    ...
    match repo.head() {
        ...
        Err(error) => Err(Error::for_system(
            Kind::GatewayError,
            format!(
                "Could not get the head for the git repository '{}'. {}",
                repo_link, error
            ),
        )),
    }
...
fn get_commit_from_head<'a>(
    ...
    match head.peel_to_commit() {
        ...
        Err(error) => Err(Error::for_system(
            Kind::GatewayError,
            format!(
                "Could not get the commit for the git repository '{}'. {}",
                repo_link, error
            ),
        )),
    }
...
fn get_tree_from_commit<'a>(commit: &'a Commit<'a>, repo_link: &'a str) -> Result<Tree<'a>, Error> {
    match commit.tree() {
        ...
        Err(error) => Err(Error::for_system(
            Kind::GatewayError,
            format!(
                "Could not get the tree for the git repository '{}'. {}",
                repo_link, error
            ),
        )),
    }
...
fn get_tree_for_process_directory_only<'a>(
    commit_tree: &'a Tree<'a>,
    git_repo: &'a Repository,
    process_directory: &'a str,
    repo_link: &'a str,
) -> Result<Tree<'a>, Error> {
    commit_tree
...
fn write_process_directory_tree_files_to_disk(
    process_directory_tree: &Tree,
    git_repo: &Repository,
    download_directory: &str,
) -> Result<(), Error> {
    for process_directory_tree_entry in process_directory_tree.iter() {
        let write_result = write_file_to_disk(
            &git_repo,
            &process_directory_tree_entry,
    ...
    match tree_entry.kind() {
        ...
        Some(ObjectType::Tree) => {
            let tree = tree_entry
                .to_object(repo)?
                .into_tree()
                .expect("Failed to convert to tree in write_file");
            for entry in tree.iter() {
                write_file_to_disk(repo, &entry, &base_path.join(tree_entry.name().unwrap()))
                    .expect("Failed to write file in tree section");
            }
        }
        _ => {}
    }
...
fn remove_clone_directory(dir_to_clone_to: &str) -> Result<(), Error> {
    ...
    match remove_result {
        ...
        Err(error) => Err(Error::for_system(
            Kind::GatewayError,
            format!(
                "Could not remove the directory '{}' which holds all the git clone files. {}",
                dir_to_clone_to, error
            ),
        )),
    }
}
