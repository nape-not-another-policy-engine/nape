...
fn retrieve_procedure_from_git_success() {
    ...
    let peer_review_py = format!(
        "{}/peer_review/at_least_two_reviewers.py",
        activity_directory_path
    );
    let requester_py = format!(
        "{}/peer_review/requester_not_a_reviewer.py",
        activity_directory_path
    );

    assert!(
        directory.paths.contains(&(
            "assurance-procedure-file".to_string(),
            nape_procedure_yaml_path.clone()
        )),
        "Expected directory to contain '{}'",
        nape_procedure_yaml_path
    );
    assert!(
        directory
            .paths
            .contains(&("activity-dir".to_string(), activity_directory_path.clone())),
        "Expected directory to contain '{}'",
        activity_directory_path
    );

    assert!(
        Path::new(&nape_procedure_yaml_path).exists(),
        "Expected file '{}' to exist.",
        nape_procedure_yaml_path
    );
    assert!(
        Path::new(&activity_directory_path).exists(),
        "Expected directory '{}' to exist.",
        activity_directory_path
    );
    assert!(
        Path::new(&peer_review_py).exists(),
        "Expected file '{}' to exist.",
        peer_review_py
    );
    assert!(
        Path::new(&requester_py).exists(),
        "Expected file '{}' to exist.",
        requester_py
    );
