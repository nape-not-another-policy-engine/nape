...
use crate::gateway_adapter::serde::specification_serializer::assurance_procedure::v1_0_0::{
    AssuranceProcedureFile, Procedure,
};
...
fn from_success() {
    ...
    let activity_1 = file
        .activities
        .iter()
        .find(|&activity| activity.name == "activity-1");
    assert!(
        activity_1.is_some(),
        "The activity-1 was not found in the file"
    );
    ...
    let action_1 = activity_1
        .actions
        .iter()
        .find(|&action| action.name == "action-1");
    ...
    let action_2 = activity_1
        .actions
        .iter()
        .find(|&action| action.name == "action-2");
    ...
    let activity_2 = file
        .activities
        .iter()
        .find(|&activity| activity.name == "activity-2");
    assert!(
        activity_2.is_some(),
        "The activity-2 was not found in the file"
    );
    ...
    let action_3 = activity_2
        .actions
        .iter()
        .find(|&action| action.name == "action-3");
    ...
    let action_4 = activity_2
        .actions
        .iter()
        .find(|&action| action.name == "action-4");
...
fn try_to_success() {
    ...
    assert!(
        result.is_ok(),
        "The conversion failed when it was expected to succeed: {:?}",
        result
    );
    ...
    assert_eq!(
        result.procedure.nrn.value,
        "nrn:sourcecode::example".to_string()
    );
    assert_eq!(result.procedure.short.value, "A Short Desc.".to_string());
    assert_eq!(
        result.procedure.description.value,
        "This is an example procedure".to_string()
    );
    assert_eq!(result.activities.list.len(), 2);

    let activity_1 = result
        .activities
        .list
        .iter()
        .find(|&activity| activity.name.value == "activity-1");
    assert!(
        activity_1.is_some(),
        "The activity-1 was not found in the result"
    );
    let activity_1 = activity_1.unwrap();
    assert_eq!(activity_1.short.value, "Short Desc - A1".to_string());
    assert_eq!(
        activity_1.description.value,
        "Long Desc - Activity 1".to_string()
    );
    assert_eq!(activity_1.actions.len(), 2);
    ...
    let action_1 = activity_1
        .actions
        .iter()
        .find(|&action| action.name.value == "action-1");
    assert!(
        action_1.is_some(),
        "The action-1 was not found in the result"
    );
    let action_1 = action_1.unwrap();
    assert_eq!(action_1.short.value, "Short Desc - A1".to_string());
    assert_eq!(
        action_1.description.value,
        "Long Desc - Action 1".to_string()
    );
    assert_eq!(action_1.test.as_str(), "test/for/action_1.txt".to_string());
    assert_eq!(
        action_1.evidence.as_str(),
        "evidence/for/action_1.txt".to_string()
    );

    let action_2 = activity_1
        .actions
        .iter()
        .find(|&action| action.name.value == "action-2");
    assert!(
        action_2.is_some(),
        "The action-2 was not found in the result"
    );
    let action_2 = action_2.unwrap();
    assert_eq!(action_2.short.value, "Short Desc - A2".to_string());
    assert_eq!(
        action_2.description.value,
        "Long Desc - Action 2".to_string()
    );
    assert_eq!(action_2.test.as_str(), "test/for/action_2.txt".to_string());
    assert_eq!(
        action_2.evidence.as_str(),
        "evidence/for/action_2.txt".to_string()
    );

    let activity_2 = result
        .activities
        .list
        .iter()
        .find(|&activity| activity.name.value == "activity-2");
    assert!(
        activity_2.is_some(),
        "The activity-2 was not found in the result"
    );
    let activity_2 = activity_2.unwrap();
    assert_eq!(activity_2.short.value, "Short Desc- A2".to_string());
    assert_eq!(
        activity_2.description.value,
        "Long Desc - Activity 2".to_string()
    );
    assert_eq!(activity_2.actions.len(), 2);
    ...
    let action_3 = activity_2
        .actions
        .iter()
        .find(|&action| action.name.value == "action-3");
    assert!(
        action_3.is_some(),
        "The action-3 was not found in the result"
    );
    let action_3 = action_3.unwrap();
    assert_eq!(action_3.short.value, "Short Desc - A3".to_string());
    assert_eq!(
        action_3.description.value,
        "Long Desc - Action 3".to_string()
    );
    assert_eq!(action_3.test.as_str(), "test/for/action_3.txt".to_string());
    assert_eq!(
        action_3.evidence.as_str(),
        "evidence/for/action_3.txt".to_string()
    );

    let action_4 = activity_2
        .actions
        .iter()
        .find(|&action| action.name.value == "action-4");
    assert!(
        action_4.is_some(),
        "The action-4 was not found in the result"
    );
    let action_4 = action_4.unwrap();
    assert_eq!(action_4.short.value, "Short Desc - A4".to_string());
    assert_eq!(
        action_4.description.value,
        "Long Desc - Action 4".to_string()
    );
    assert_eq!(action_4.test.as_str(), "test/for/action_4.txt".to_string());
    assert_eq!(
        action_4.evidence.as_str(),
        "evidence/for/action_4.txt".to_string()
    );
}
...
fn generate_valid_assurance_procedure() -> AssuranceProcedure {
    let activity_1 =
        Activity::new("activity-1", "Short Desc - A1", "Long Desc - Activity 1").unwrap();
    let activity_1 = activity_1.add(
        Action::builder()
            .name("action-1")
    ...
    let activity_2 =
        Activity::new("activity-2", "Short Desc- A2", "Long Desc - Activity 2").unwrap();
    let activity_2 = activity_2.add(
        Action::builder()
            .name("action-3")
...
fn generate_valid_assurance_procedure_file() -> AssuranceProcedureFile {
    let action1 = FileAction {
        ...
        short: "Short Desc - A1".to_string(),
        description: "Long Desc - Action 1".to_string(),
        test: "test/for/action_1.txt".to_string(),
        evidence: "evidence/for/action_1.txt".to_string(),
    };
    let action2 = FileAction {
        name: "action-2".to_string(),
        short: "Short Desc - A2".to_string(),
        description: "Long Desc - Action 2".to_string(),
        test: "test/for/action_2.txt".to_string(),
        evidence: "evidence/for/action_2.txt".to_string(),
    };
    let action3 = FileAction {
        name: "action-3".to_string(),
        short: "Short Desc - A3".to_string(),
        description: "Long Desc - Action 3".to_string(),
        test: "test/for/action_3.txt".to_string(),
        evidence: "evidence/for/action_3.txt".to_string(),
    };
    let action4 = FileAction {
        name: "action-4".to_string(),
        short: "Short Desc - A4".to_string(),
        description: "Long Desc - Action 4".to_string(),
        test: "test/for/action_4.txt".to_string(),
        evidence: "evidence/for/action_4.txt".to_string(),
    };

    let activity1 = FileActivity {
        name: "activity-1".to_string(),
        short: "Short Desc - A1".to_string(),
        description: "Long Desc - Activity 1".to_string(),
        actions: vec![action1, action2],
    };
    let activity2 = FileActivity {
        name: "activity-2".to_string(),
        short: "Short Desc- A2".to_string(),
        description: "Long Desc - Activity 2".to_string(),
        actions: vec![action3, action4],
    };

    let procedure = Procedure {
        nrn: "nrn:sourcecode::example".to_string(),
        short: "A Short Desc.".to_string(),
        description: "This is an example procedure".to_string(),
    };

    AssuranceProcedureFile {
        api_version: "1.0.0".to_string(),
        kind: "AssuranceProcedure".to_string(),
        procedure,
        activities: vec![activity1, activity2],
    }
}
...
fn generate_invalid_assurance_procedure_file_bad_api_version() -> AssuranceProcedureFile {
    ...
    AssuranceProcedureFile {
        api_version: "bad version".to_string(),
        kind: "AssuranceProcedure".to_string(),
        procedure,
        activities: vec![activity1, activity2],
    }
