...
pub fn app_state_from_nape_config() -> Result<CLIAppState, Error> {
    let local_state_file = nape_cli_config_file_path().map_err(|error| {
        Error::for_system(
            Kind::NotFound,
            format!(
    ...
    let app_state =
        retrieve_from_filesystem(&local_state_file, deserialize_from_yaml).map_err(|error| {
            Error::for_system(
                Kind::ProcessingFailure,
                format!(
                    "Failed to deserialize the local NAPE state configuration file. {}",
                    error.message
                ),
            )
        })?;
